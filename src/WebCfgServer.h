...
private:
    #ifndef NUKI_HUB_UPDATER
    esp_err_t sendSettings(PsychicRequest *request, PsychicResponse *response);
    ...
    esp_err_t buildConfirmHtml(PsychicRequest *request, PsychicResponse *resp, const String &message, uint32_t redirectDelay = 5, bool redirect = false);
    esp_err_t buildOtaHtml(PsychicRequest *request, PsychicResponse *resp, bool debug = false);
    esp_err_t sendCss(PsychicRequest *request, PsychicResponse *response);
    esp_err_t sendFavicon(PsychicRequest *request, PsychicResponse *response);
    void createSsidList();
