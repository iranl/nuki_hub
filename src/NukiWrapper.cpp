...
LockActionResult NukiWrapper::onLockActionReceivedCallback(const char *value)
{
    ...
    {
        if(!nukiLockPreferences->getBool(preference_official_hybrid_actions, false) || !networkInst->_offConnected) nukiInst->_nextLockAction = action;
        else networkInst->publishInt(mqtt_topic_official_lockActionEvent, (int)action);
...
void NukiWrapper::gpioActionCallback(const GpioAction &action, const int& pin)
{
    ...
    switch(action)
    {
        case GpioAction::Lock:
            if(!nukiLockPreferences->getBool(preference_official_hybrid_actions, false) || !networkInst->_offConnected) nukiInst->lock();
            else networkInst->publishInt(mqtt_topic_official_lockActionEvent, 2);
            break;
        case GpioAction::Unlock:
            if(!nukiLockPreferences->getBool(preference_official_hybrid_actions, false) || !networkInst->_offConnected) nukiInst->unlock();
            else networkInst->publishInt(mqtt_topic_official_lockActionEvent, 1);
            break;
        case GpioAction::Unlatch:
            if(!nukiLockPreferences->getBool(preference_official_hybrid_actions, false) || !networkInst->_offConnected) nukiInst->unlatch();
            else networkInst->publishInt(mqtt_topic_official_lockActionEvent, 3);
            break;
        case GpioAction::LockNgo:
            if(!nukiLockPreferences->getBool(preference_official_hybrid_actions, false) || !networkInst->_offConnected) nukiInst->lockngo();
            else networkInst->publishInt(mqtt_topic_official_lockActionEvent, 4);
            break;
        case GpioAction::LockNgoUnlatch:
            if(!nukiLockPreferences->getBool(preference_official_hybrid_actions, false) || !networkInst->_offConnected) nukiInst->lockngounlatch();
            else networkInst->publishInt(mqtt_topic_official_lockActionEvent, 5);
            break;
    }
